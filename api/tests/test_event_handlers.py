...
class TestDemandaEventHandler:
    ...
    def test_handle_demanda_with_sufficient_inventory(self):
        ...
        context = SimulationContext(r=10, Q=20, inventario=10, configuracion=configuracion)  # Inventario inicial de 10
        ...
        assert context.costo_total_faltante == 0.0  # No hay faltante
    
    def test_handle_demanda_with_insufficient_inventory(self):
        handler = DemandaEventHandler()
        evento = EventoDemanda(10, 8)  # Demanda de 8 unidades
        configuracion = ConfiguracionSimulacion.from_parametros(**PARAMS)
        context = SimulationContext(r=10, Q=20, inventario=3, configuracion=configuracion)  # Inventario inicial de 3
        
        handler.handle(evento, context)
        
        # Se vendieron 3 unidades (min entre demanda 8 e inventario 3)
        assert context.obtener_inventario() == 0  # 3 - 3
        assert context.ingresos == 300.0  # 3 * 100
        assert context.costo_total_faltante == 250.0  # 5 faltantes * 50
...
class TestLlegadaPedidoEventHandler:
    ...
    def test_handle_llegada_pedido(self):
        ...
        configuracion = ConfiguracionSimulacion.from_parametros(**PARAMS)
        context = SimulationContext(r=10, Q=20, inventario=5, configuracion=configuracion)  # Inventario inicial de 5
